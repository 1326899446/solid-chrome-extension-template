...
async function call_app_android(api,params,callback){
  ...
  if(api==='reqxml'){
    ...
    req_data.apiPath = `/${api}?${typeof params === 'string' ? params : new URLSearchParams(params).toString()}`;
  }
  const xhr = new XMLHttpRequest();
  xhr.open('POST', req_data.apiPath, true);
  xhr.setRequestHeader('Content-Type', req_data.contentType || 'application/x-www-form-urlencoded');
  xhr.send(send_data);
